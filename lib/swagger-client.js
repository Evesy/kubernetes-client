...
class Root extends Component {
  ...
  addCustomResourceDefinition (manifest) {
    ...
    const addSpec = (versionSpec) => {
      const version = versionSpec.name
      const spec = { paths: {} }
      //
      ...
      if (versionSpec?.subresources && versionSpec.subresources.status) {
        const statusPath = `/apis/${group}/${version}${namespace}/${name}/{name}/status`
        spec.paths[statusPath] = ['get', 'put', 'patch'].reduce((acc, method) => {
          acc[method] = { operationId: `${method}Template${name}` }
          return acc
        }, {})
      }
      ...
      if (versionSpec?.subresources && versionSpec.subresources.scale) {
    ...
    if (manifest.spec.version) {
      versions.push({ name: manifest.spec.version })
    }
    versions.forEach(version => addSpec(version))
  }
}
...
class Client {
  constructor (options) {
    ...
    if (!backend) {
      throw new Error('Please inject a backend')
    }
...
class Client1_17 extends Client {
  constructor (options) {
    super(Object.assign({}, options, { version: '1.17' }))
  }
}
